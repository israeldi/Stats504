    ...
    for female in 0, 1:
        for bmi in 22, 26, 31:
            ...
            dcompare = dx[(dx["RIDRETH1"] == "OH") & (dx["RIDAGEYR"] >= 18) & 
                          ...
                          (dx["BMXBMI"] < bmi+2) & (dx["BMXBMI"] > bmi-2)].copy()
            
            #ax.axes([0.1, 0.1, 0.66, 0.8])
            ax.grid(True)
            ax.plot(dcompare["RIDAGEYR"], dcompare["BPXSY1"], 'o')
            ax.set_xlabel("Age (years)", size=15)
            ax.set_ylabel("BP (mm/Hg)", size=15)
...
def plot_fit_by_bmi(result, fml):
    ...
    for female in 0, 1:
        for age in 25, 50, 75:
            ...
            la += ", age=%.0f" % age
            ax = eval("ax%s" % (i)) 
            ax.plt.plot(da.BMXBMI, pr, '-', label=la)
            i += 1
...
out_data_path = "./data/clean_nhanes.parq"
dx = pd.read_parquet(out_data_path)

# Main effects only (additive, linear model)
fml0 = "BMXBMI ~ RIDAGEYR + Female + RIDRETH1 + SLD012"
